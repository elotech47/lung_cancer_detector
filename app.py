...
def loadModel():
    ...
    #model1 = load_model('Models\\model_file.h5')
    model = load_model('Models\\completeModel.h5')
...
def homePage():
    ...
    st.title("Numbers AI Mobility Health care")
    st.title("An AI-Based Disease Diagnostics")
    st.text("Built by Numb3rs input and output technology")
    
def welcomePage():
    st.subheader("Welcome to Numbers AI Mobility Health care Tool")
...
def NotFound():
    st.error("!!!Model under development")
    from PIL import Image
    img = Image.open("map_Nig.JPG")
    st.image(img, width=300,caption="Model")


def main():
    #homePage()
    #
    activities1 = ["Welcome", "Diagnose Disease", "Questionnaire", "Help"]
    choice1 = st.sidebar.selectbox("Select Activty", activities1, key = 1)
    
    activities2 = ["Select Here","Covid-19","Diabetis Retinopathy","Malaria", "Pneumonia", "Tuberculosis", "Lassa Fever", "Breast Cancer", "Heart Disease"]
    choice2 = st.sidebar.selectbox("Select the Disease you want to diagnose", activities2,  key = 2)

    if choice1 == "Welcome":
        homePage()
        welcomePage()

    if choice1 == "Diagnose Disease":
        homePage()
        st.info("Please select the Disease you want to Diagnose from the dropdown")

        if choice2 == "Diabetis Retinopathy":
            st.subheader("Upload Eye Scan")
            image_file = st.file_uploader(
                "Upload Image", type=['jpg', 'png', 'jpeg'])
            if image_file is not None:
                SampleImg = Image.open(image_file)
                st.text("Eye Image")
                st.success("Successful loaded Eye Scan Image")
                is_check = st.checkbox("Display Image")
                if is_check:
                    st.image(SampleImg, width=300)
            ...
            if name == None:
                ...
                if st.button("Diagnose"):
                    ...
                    if covid >= normal:
                        ...
                        st.write("Covid-19 Suspected with {:.2f} percent certainty".format(prob))
                        st.write("[INFO] {}".format(label))
                    else:
                        imageID, label, prob = [1, "normal", normal*100]
                        label = "{}: {:.2f}%".format(label, prob)
                        st.write("Normal Condition Suspected with {:.2f} percent certainty".format(prob))
                        st.write("[INFO] {}".format(label))
                    ...
                    name_save = "Images\{}.jpeg".format(name)
                    nameDetailed = "Images\{}_detailed.jpeg".format(name)
                    model = loadModel()
                    cam = GradCAM(model, i)
                    Reimage = cv2.cvtColor(np.float32(SampleImg), cv2.COLOR_BGR2RGB)
                    Reimage = cv2.resize(Reimage, ( 224, 224))
                    ReimageGrad = np.array(Reimage)/255.0
                    ReimageGrad = np.expand_dims(ReimageGrad, axis=0)
                    imageArray = np.array(SampleImg)
